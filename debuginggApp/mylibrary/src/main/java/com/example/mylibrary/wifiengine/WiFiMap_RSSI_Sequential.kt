...
class WiFiMap_RSSI_Sequential constructor(wiFiDataMap: WiFiDataMap, map_hand: MagneticFieldMap) {
    ...
    private var wifi_particle_mother_list = arrayListOf<WifiParticleMother>()
...
//            angleList = ((360-cur_gyro)-90..(360-cur_gyro)+90 step angle_step_num).toList().toIntArray()
            ...
            cur_step = -1
            wifi_particle_mother_list = arrayListOf<WifiParticleMother>()
            instant_result = mutableMapOf("status_code" to -1.0f, "gyro_from_map" to -1.0f, "pos_x" to -1.0f, "pos_y" to -1.0f)
        }
...
//                matchingChildren(particle_mother)
                if (particle_mother.particleChildrenList.size == 0) {
    ...
    fun first_find_particles(ans_range : ArrayList<Float>){
        ...
        for (i in angleList.indices){
            wifi_particle_mother_list.add(WifiParticleMother(angleList[i]))
...
//        return arrayListOf(ans_range[0], ans_range[1], ans_range[2], ans_range[3])
    ...
    fun moveChildren(particle_mother: WifiParticleMother, step_length : Double, gyro : Float) {
        var cur_idx = -1
        while (true) {
            var gyro_result = gyro
            cur_idx += 1
            if (cur_idx == particle_mother.particleChildrenList.size) {
                break
            }
            var children = particle_mother.particleChildrenList[cur_idx]

            children.x -= (step_length * 10 * sin((particle_mother.myAngle - gyro_result).toFloat() * PI / 180)).toFloat()
            children.y += (step_length * 10 * cos((particle_mother.myAngle - gyro_result).toFloat() * PI / 180)).toFloat()
...
//        }
    ...
    private fun matchingChildren(particle_mother: WifiParticleMother) {
        var cur_idx = -1
        while (true) {
...
//            }
    ...
    private fun calculate_answer_position(mother : WifiParticleMother) : ArrayList<Double>{
        var answer_x = 0.0
        var answer_y = 0.0
        ...
        var num_of_children = mother.particleChildrenList.size
        ...
        dist_avg = dist_avg / mother.particleChildrenList.size
        if (dist_avg > 3.0) {
    ...
    private fun estimateInitialDirAndPos(mother_list: List<WifiParticleMother>, gyro: Float): MutableMap<String, Float> {
        ...
        lateinit var best_mother : WifiParticleMother
...
//
//                area_check_pos_list = arrayListOf<Array<Float>>()
//                for (c in best_mother.particleChildrenList) {
...
//                }
        ...
        // 혹시 모를 에러를 방지
        var num_of_children = best_mother.particleChildrenList.size
        ...
        var answer_dir = ((((360 - (best_mother.myAngle).toInt()) + gyro) + 360) % 360)
