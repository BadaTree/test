...
//            angleList = ((360-cur_gyro)-90..(360-cur_gyro)+90 step angle_step_num).toList().toIntArray()
    ...
    fun getLocation(wifi_string: String, stepLength: Double, gyro_for_pf: Float, gyro: Float, pf_result: Array<Double>) : Pair<MutableMap<String, Float>, Int> {

        if(!foundpos) {
            var (instant_result_, unqWifi_) = getlocation_WF(wifi_string, stepLength, gyro, gyro_for_pf)
    ...
    fun getlocation_WF(wifi_string: String, stepLength: Double, gyro: Float, gyro_for_pf:Float) : Pair<MutableMap<String, Float>, Int> {
        ...
        var unqWifi = vectorcompare(wifi_string, cur_step)
        if(foundpos){
...
//                matchingChildren(particle_mother)
    ...
    fun vectorcompare(wifi_string: String, cur_step : Int): Int{
        ...
        for (i in pos){
            ...
            for (j in 0 .. wifilistsize - 1 step(1)){
                try {
                    ...
                    Log.d("wifi rssi error", "error at :" + wifi.toString())
                }
            }
        ...
        var wifimap_cnt_list = wifilist.size
        var compare_val = 0
        ...
        Log.d("comm", (unq_cnt_list[0]).toString())
        Log.d("wifimap_cnt", ((unq_cnt_list[0]/ wifimap_cnt_list) * 100).toString())
        return (unq_cnt_list[0]/ wifimap_cnt_list)*100
    }
...
//                }
        ...
        if (answer_position_xy == arrayListOf(-1.0, -1.0)) {
            ...
            // 200.0f -> IL 완료. 완전 수렴
            return Pair(mutableMapOf("status_code" to 200.0f, "gyro_from_map" to answer_dir, "pos_x" to answer_x, "pos_y" to answer_y),particle_num)
