...
internal class ParticleFilter constructor(map: MagneticFieldMap, particleCount : Int, posX : Int, posY : Int, maxR : Int) {
    private val map: MagneticFieldMap = map
    ...
    init {
        ...
        for (i in 0 until particleCount) {
            do {
                x = rnd.nextInt(2 * rR) + posX - rR
                y = rnd.nextInt(2 * rR) + posY - rR
//                count++
//                if (count>30) {
//                    rR += 10
//                    count = 0
//                }
    ...
    private fun blocking(x: Double, y: Double, stepLength: Double) {
        var count = 0
        var area = round((stepLength * 10) * 6).toInt()      //round((stepLength * 10) * 4).toInt()
        if (area==0) area = 30
        particles.forEach { p ->
            while (!map.isPossiblePosition(p.x, p.y)){
                Log.d("forloopcheck","check")
