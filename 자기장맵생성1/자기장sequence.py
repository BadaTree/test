...
from modules import AutoPDRScaler
...
if __name__ == "__main__":
    ...
    #파일 행 수 체크
    rawdata1 = pd.read_csv('./rawdata/' +file_name +'.txt',
    sep = "\t", engine = 'python', encoding = "cp949", header = None)

    file_name = '30'

    rawdata2 = pd.read_csv('./rawdata/' + file_name + '.txt',
                           sep="\t", engine='python', encoding="cp949", header=None)
    ...
    plt.axis('on')
    plt.grid('True')
    plt.plot(rawdata1.iloc[:20, 2])
    plt.plot(rawdata2.iloc[:20, 2])
    #colorbar range 조절
    # plt.clim(-10,50)
    # plt.colorbar()
    plt.show()

    # print(np.max(rawdata.iloc[:,0]),np.max(rawdata.iloc[:,1]))
    # #수집 한 줄 간격
    ...
    #
    #     scaler1 = AutoPDRScaler.Scaler(sensor_df=df1)
    #     interpolated_byx_array[idx, :] = scaler1.scaled_df
    #
    ...
    #          2: tempmagx.iloc[:, idx*6], 3: tempmagy.iloc[:, idx*6], 4: tempmagz.iloc[:, idx*6]})
    #     scaler2 = AutoPDRScaler.Scaler(sensor_df=df1)
    #     interpolated_byy_array[idx, :] = scaler2.scaled_df
    #
    #
    ...
    #
    # #맵저장
    # total_magx.to_csv('./onlyinterpol/magx.txt', index=False, header=None, sep="\t")
    # total_magy.to_csv('./onlyinterpol/magy.txt', index=False, header=None, sep="\t")
    # total_magz.to_csv('./onlyinterpol/magz.txt', index=False, header=None, sep="\t")
    #
    # # path_save = open('./onlyinterpol/hashmap.txt', 'w')
    #
    # # for 저장
    ...
    # plt.axis('on')
    # plt.grid('True')
    # plt.imshow(total_magz, cmap='jet', interpolation='none')
    # #colorbar range 조절
    # # plt.clim(-10,50)
    # plt.colorbar()
    # plt.show()
