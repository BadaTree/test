...
class ExIndoorLocalization {
    ...
    constructor(magneticStream : InputStream?, magneticStreamForInstant : InputStream?, wifiStreamTotal : InputStream?, wifiStreamRSSI : InputStream?, wifiStreamUnq : InputStream?, inputStreamOfBuilding : InputStream) {
...
//                    )
    ...
    */
    ...
    // In pocket & Hans swing 대응 // (원준)
    private var steplength_for_pf = 0.0
    private var gyro_cali_value_for_in_pocket : Float = 0.0f
    private var gyro_for_pf = 0.0
    private var pre_pos : Int = 0
    private var in_pocket_stable : Boolean = false
    private var in_pocket_stable_cnt : Int = 0
    private var step_num_in_stabel : Int = 0
    ...
    private var pressureMoving20Average: SuminAverage = SuminAverage(20)
    private var preCnt : Int = 0
            ...
            when (event!!.sensor.type) {
                ...
                Sensor.TYPE_PRESSURE -> {
                    ...
                    if(moveInfrontLocate) {
                        ...
                        if(preCnt == 0 && pressure[0].toDouble() > 0.0) {
                            firstP = pressure[0].toDouble()
                            preCnt++
                        }
...
//
                        if((elvPressureGradientaverage20*1000 < -4 || elvPressureGradientaverage20*1000 > 4 ) && !getInZone) {
                            ...
                            if ((infoType == "EL2Front" || infoType == "EL2Front2") && startfloor == 0) {
                                elvX = 170.0
                                elvY = 43.0
                            }
                            ...
                            if( (infoType == "EL3Front" || infoType == "EL3Front2"  || infoType == "EL3Front3")  && startfloor == 0 ) {
                                elvX = 163.0
                                elvY = 1280.0
                            }
                            ...
                            if( (infoType == "EL4Front" || infoType == "EL4Front2" ) && startfloor == 0) { //지하 1층
                                elvX = 163.0
                                elvY = 1340.0
                                makeElvIn = true
                            }

                            if((infoType == "EL4Front" || infoType == "EL4Front2" ) && startfloor != 0) {
                                elvX = 70.0
                                elvY = 65.0
                                makeElvIn = true
                            }

                            movingFloor.getCondition(infoType, startfloor , true, firstP )
                        ...
                        if(!moveEnd) {
                            if ((ILResult["status_code"] == 200.0f) || (ILResult["status_code"] == 202.0f)) {
                                ...
                                if (devicePosture == 0) {
                                    ...
                                    getGyroscope.save_gyro()
                                }
//                            // In pocket & Hand swing 대응
                                else {
                                    if ((ILResult["status_code"] == 400.0f) && devicePosture == 1) { // In pocket
                                        steplength_for_pf = pdrResult.stepLength
                                        if (posture_changed(devicePosture, pre_pos)){
                                            in_pocket_stable_cnt = 0
                                            in_pocket_stable = false
                                            gyro_for_pf = getGyroscope.get_pre_gyro_for_in_pocket(0).toDouble()

                                        }
                                        else {
                                            if (in_pocket_stable == false){
                                                in_pocket_stable_cnt += 1
                                                if (in_pocket_stable_cnt > 4 ) {
                                                    in_pocket_stable = true
                                                }
                                                gyro_for_pf = getGyroscope.get_pre_gyro_for_in_pocket(0).toDouble()
                                            }
                                            else {
                                                if (in_pocket_stable_cnt == 5){
                                                    gyro_cali_value_for_in_pocket = gyro_value_map_collection
                                                    step_num_in_stabel = stepCount
                                                }
                                                in_pocket_stable_cnt += 1
                                                // 짝수 또는 홀수 걸음일 때만 자이로 사용
                                                if ((stepCount - step_num_in_stabel) % 2 == 0) {
                                                    gyro_for_pf = getGyroscope.get_pre_gyro_for_in_pocket(0) + (gyro_value_map_collection - gyro_cali_value_for_in_pocket.toDouble())
                                                    Log.d("gyro_for_pf", gyro_for_pf.toString())

                                                }
                                            }
                                        }
                                    }
                                    else if ((ILResult["status_code"] == 400.0f) && devicePosture == 2) { // Hand swing
                                        steplength_for_pf = pdrResult.stepLength
                                        gyro_for_pf = gyro_value_map_collection.toDouble()
                                    }


                                    PFResult = particleFilter.step(caliVector, gyro_for_pf, steplength_for_pf)

                                }

                            }
                        } else {
                            PFResult = particleFilter.step(caliVector, gyro_value_map_collection.toDouble(), pdrResult.stepLength + 0.04
                            )
                        }
                        ...
                        if (particleOn) {
                            // -----------------------------------Sumin---------------------------------------------------------
                                ...
                                if(buildingInfoResult != null) {
                                    ...
                                    if(!getInZone && (buildingInfoResult!!.type == "EL1Front" || buildingInfoResult!!.type == "EL2Front" || buildingInfoResult!!.type == "EL2Front2"
                                                || buildingInfoResult!!.type == "EL3Front" || buildingInfoResult!!.type == "EL3Front2" || buildingInfoResult!!.type == "EL3Front3"
                                                || buildingInfoResult!!.type == "EL4Front" || buildingInfoResult!!.type == "EL4Front2" )) {

                                        elvPressure  = ElvPressure(3, 0.005)
                                        pressureMoving20Average = SuminAverage(20)
                                    ...
                                    if (!getInZone && (buildingInfoResult!!.type != "EL1Front") && (buildingInfoResult!!.type != "EL2Front")
                                        && (buildingInfoResult!!.type != "EL2Front2") && (buildingInfoResult!!.type != "EL3Front") && (buildingInfoResult!!.type != "EL3Front2")
                                        && (buildingInfoResult!!.type != "EL3Front3") && (buildingInfoResult!!.type != "EL4Front") && (buildingInfoResult!!.type != "EL4Front2")) {
                                        ...
                                        movingFloor.getCondition(infoType, startfloor, false , firstP )
                                    ...
                                    if(zoneCheckEnd) {
                                        ...
                                        if ((buildingInfoResult!!.type != "EL1") && ( buildingInfoResult!!.type != "EL2") && (buildingInfoResult!!.type != "EL3") && (buildingInfoResult!!.type != "EL4")) {

                                            Toast.makeText(acontext, "엘리베이터에서 하차하였습니다. ", Toast.LENGTH_SHORT).show()
                                            zoneCheckEnd = false
                                            getInZone = false
                                        }
                                    }
                            ...
                            if (checkMovingFloor) {
                                ...
                                if (movingFloor.isArrival) {
                                    ...
                                    if(startfloor != 0 && (infoType == "EL1" || infoType == "EL2"|| infoType == "EL1Front"
                                                || infoType == "EL2Front" || infoType == "EL2Front2" )) {
                                        ...
                                        particleFilter = ParticleFilter(mapAfterFloorChange, 100, round(finalArrival[0]).toInt(), round(finalArrival[1]).toInt(), 1)
                                    }

                                    if (startfloor != 0 && infoType == "EL3" || infoType == "EL4" || infoType == "EL3Front" || infoType == "EL3Front2" || infoType == "EL3Front3" || infoType == "EL4Front" || infoType == "EL4Front2" ) {
...
//                                    getInZone = false
        ...
        pre_pos = devicePosture
        returnGyro = gyro_value_map_collection.toString()
        if (getPose() == "In Pocket") {
            returnGyro = gyro_for_pf.toString()
        }
    ...
    private fun posture_changed(cur_pos : Int, pre_pos : Int): Boolean {
        var result = false
        if (cur_pos != pre_pos)
            result = true
        else
