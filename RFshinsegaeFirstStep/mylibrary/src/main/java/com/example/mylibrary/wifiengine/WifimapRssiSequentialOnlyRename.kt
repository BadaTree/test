...
class WifimapRssiSequentialOnlyRename constructor(wiFiDataMap: WiFiDataMap, map_hand: MagneticFieldMap) {
    ...
    fun getLocation(wifiString: String, stepLength: Double, gyro_for_pf: Float, gyro: Float, pf_result: Array<Double>) : MutableMap<String, Float> {
    ...
    private fun getlocationWF(wifiString: String, stepLength: Double, gyro: Float, gyro_for_pf:Float) : MutableMap<String, Float>{
        ...
        vectorcompare(wifiString, curStep)


        if(foundpos){
//            universalRange = moveRange(ansRange, universalRange, stepLength, gyro_for_pf)
            universalRange = ansRange
//            universalRange = moveRange2(ansRange, universalRange, stepLength)
...
//            firstFindArea(ansRange)
        ...
        if(!foundpos) {
            if (curStep == 0) {
                if (foundpos) {
                    firstFindParticles(universalRange2)
                } else {
                    firstFindParticles(
                        arrayListOf(
...
//                matchingChildren(particleMother)
    ...
    private fun vectorcompare(wifiString: String, curStep : Int){
        wifidata = wifiString
        ...
        var rangeIdx = rangeThres
        var rssiRangeIdx = rssiRangeNum
        ...
        compareVal = if (curStep == 0){
            rangeThres
        }else{
            secondRangeThres
        }

        rangeIdx = if (unqCntList.size <= compareVal){
            unqCntList.size - 1
        } else{
            compareVal
        }
        ...
        rssiRangeIdx = if (unqRssiDiffList.size <= compareVal){
...
//                }
        ...
        val answerDir = ((((360 - (bestMother.myAngle).toInt()) + gyro) + 360) % 360)
